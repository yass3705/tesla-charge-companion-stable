...
const norm=v=>String(v==null?'':v).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/\bionity\b/g,' ').replace(/[^a-z0-9]+/g,' ').trim().replace(/\s+/g,' ');
...
const national=rows.filter(r=>norm(r?.[5])==='ionity').map(r=>({stationId:String(r[0]),name:String(r[1]||''),address:String(r[2]||''),lat:Number(r[3]),lon:Number(r[4])})).filter(x=>Number.isFinite(x.lat)&&Number.isFinite(x.lon));
...
for(const src of sources){
  ...
  const srcName=norm(src.name),natName=norm(first.n.name),exactName=srcName&&srcName===natName;
  const srcAddress=norm(`${src.address} ${src.city}`),natAddress=norm(first.n.address),addressOverlap=srcAddress&&natAddress&&(srcAddress.includes(natAddress)||natAddress.includes(srcAddress));
...
const duplicateNational=[...new Map(strict.map(x=>[x.nationalId,strict.filter(y=>y.nationalId===x.nationalId)])).entries()].filter(([,v])=>v.length>1).map(([nationalId,v])=>({nationalId,count:v.length,sourceIds:v.map(x=>x.locationId)}));
const result={nationalIonityStations:national.length,sourceIonityLocations:sources.length,buckets,strictUniqueCandidates:strict.length-duplicateNational.reduce((n,x)=>n+x.count,0),duplicateNational,strictSamples:strict.slice(0,20),nearestSamples:candidates.slice().sort((a,b)=>a.distanceM-b.distanceM).slice(0,20),worstNearest:candidates.slice().sort((a,b)=>b.distanceM-a.distanceM).slice(0,12)};
