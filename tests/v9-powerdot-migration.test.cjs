...
assert.deepEqual(normalized.offerRules[0].evseIds,['FR*POD*E123*1']);
...
const good={canonicalId:'FR:national:GOOD',sourceStationId:'GOOD',countryCode:'FR',name:'Powerdot exact',address:'',latitude:48.8,longitude:2.1,physicalOperator:{id:'powerdot',name:'Powerdot'},evses:[{id:'FR*POD*E123*1',pdcIds:['FR*POD*E123*1'],connectors:[{id:'c1',kind:'DC',powerKw:120}]}]};
const bad={canonicalId:'FR:national:BAD',sourceStationId:'BAD',countryCode:'FR',name:'Powerdot other',address:'',latitude:48.81,longitude:2.11,physicalOperator:{id:'powerdot',name:'Powerdot'},evses:[{id:'FR*POD*OTHER*1',pdcIds:['FR*POD*OTHER*1'],connectors:[{id:'c2',kind:'DC',powerKw:120}]}]};
...
(async()=>{
  ...
  assert.equal(exact.offers.filter(o=>o.provider==='Powerdot direct').length,1,'exact PDC must receive Powerdot direct tariff');
  ...
  assert.equal(offer.metadata.identityMode,'exact_irve_pdc');
