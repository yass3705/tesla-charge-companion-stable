...
for(const rule of directRules){
  ...
  if(pdcs.length){
    for(const pdc of pdcs){
      if(!directByPdc.has(pdc))directByPdc.set(pdc,[]);
      directByPdc.get(pdc).push(rule);
    }
  }else if(stations.length)directStationRules.push(rule);
  else directScopedRules.push(rule);
}

function stationTokens(stationId){
  return new Set([stationId,`FR:national:${stationId}`,`irve-station:${stationId}`,`national:FR:${stationId}`,...(aliasesByStation.get(stationId)||[])]);
}
...
function scopedRuleMatches(rule,{operator,network,kind,power}){
  ...
  const min=Number(rule.minPowerKw),max=Number(rule.maxPowerKw);
  if(Number.isFinite(min)&&power<min)return false;if(Number.isFinite(max)&&power>max)return false;
  return true;
}
function directMatches(stationId,pdc,ctx){
  const exact=(directByPdc.get(pdc)||[]).filter(r=>scopedRuleMatches(r,ctx));
  if(exact.length)return exact;
  const tokens=stationTokens(stationId);
  const station=directStationRules.filter(r=>exactStationRuleMatches(r,tokens)&&scopedRuleMatches(r,ctx));
  if(station.length)return station;
  return directScopedRules.filter(r=>scopedRuleMatches(r,ctx));
}

const stats={stations:nationalRows.length,pdcs:0,withDirect:0,withElectroverse:0,withElectra:0,withAnyEmsp:0,withDirectAndEmsp:0,withFallbackOnly:0,withoutAnyKnownTariff:0};
...
for(const row of nationalRows){
  ...
  for(const cfg of configs){
    ...
    for(const pdc of pdcs){
      stats.pdcs++;
      const direct=directMatches(stationId,pdc,{operator,network,kind,power});
      const providers=emspByPdc.get(pdc)||new Set();
      const hasDirect=direct.length>0,hasEv=providers.has('electroverse'),hasElectra=providers.has('electra'),hasEmsp=hasEv||hasElectra,hasFallback=pricing.length>0;
      if(hasDirect)stats.withDirect++;
      if(hasEv)stats.withElectroverse++;
      if(hasElectra)stats.withElectra++;
      if(hasEmsp)stats.withAnyEmsp++;
      if(hasDirect&&hasEmsp)stats.withDirectAndEmsp++;
      if(!hasDirect&&!hasEmsp&&hasFallback)stats.withFallbackOnly++;
      if(!hasDirect&&!hasEmsp&&!hasFallback)stats.withoutAnyKnownTariff++;
      for(const r of direct){const sid=r._sourceId||r.sourceId||'direct';sourceHits.set(sid,(sourceHits.get(sid)||0)+1);}
      const key=operator||'Unknown',s=operatorStats.get(key)||{operator:key,pdcs:0,direct:0,emsp:0,fallbackOnly:0,none:0};
      s.pdcs++;if(hasDirect)s.direct++;if(hasEmsp)s.emsp++;if(!hasDirect&&!hasEmsp&&hasFallback)s.fallbackOnly++;if(!hasDirect&&!hasEmsp&&!hasFallback)s.none++;operatorStats.set(key,s);
    }
  }
}
...
const result={
  generatedFrom:{nationalGeneratedAt:nationalManifest.generatedAt,emspGeneratedAt:emspManifest.generatedAt||null,nationalStations:nationalManifest.stationCount,nationalPdcCount:nationalManifest.pdcCount},
  exactPdcAudit:{...stats,percent:{direct:pct(stats.withDirect),electroverse:pct(stats.withElectroverse),electra:pct(stats.withElectra),anyEmsp:pct(stats.withAnyEmsp),directAndEmsp:pct(stats.withDirectAndEmsp),fallbackOnly:pct(stats.withFallbackOnly),noKnownTariff:pct(stats.withoutAnyKnownTariff)}},
  directSourcePdcHits:Object.fromEntries([...sourceHits.entries()].sort((a,b)=>b[1]-a[1])),
  topCoverageGapsByPhysicalOperator:topGaps
};
