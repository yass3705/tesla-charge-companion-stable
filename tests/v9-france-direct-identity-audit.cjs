...
const nationalPdcs=new Set(),normalizedPdcs=new Set(),stationTokens=new Set(),operatorStations=new Map();
...
function rulesFor(source,payload){
  if(!payload)return[];
  if(source.adapter==='direct-offer-json')return Direct.normalizePayload(payload).offerRules||[];
  if(source.adapter==='direct-station-gzip')return LegacyStations.normalizePayload(payload,source).offerRules||[];
  return[];
}
function rawDiagnostic(source,payload){
  if(!payload)return null;
  if(source.id==='powerdot-direct-france'){
    const first=(payload.chargers||[])[0]||null;
    return{rootKeys:Object.keys(payload).sort(),sourceType:payload.sourceType||null,operator:payload.operator||null,chargerCount:Array.isArray(payload.chargers)?payload.chargers.length:null,firstChargerKeys:first?Object.keys(first).sort():[],firstLocationKeys:first?.location?Object.keys(first.location).sort():[],firstNestedChargerKeys:first?.charger?Object.keys(first.charger).sort():[],firstIrvePdcIds:first?.irvePdcIds||null};
  }
...
function summarize(source){
  ...
  return{
    ...
    rawDiagnostic:rawDiagnostic(source,payload)
  };
}
