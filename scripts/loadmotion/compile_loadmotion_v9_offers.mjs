...
function baseRule(){return{scope:'allDay',start:'00:00',end:'24:00',billing:'kwh',currency:'EUR',pricePerKwh:0,chargePerMinute:0,connectionFee:0,idlePerMinute:0,afterMinutesRate:0,afterMinutesThreshold:0,afterMinutesCap:0,afterMinutesCapStart:'00:00',afterMinutesCapEnd:'24:00'};}
function ruleFromDefinition(def){
  ...
  if(dims.flatFee?.active)r.connectionFee=round(dims.flatFee.price);
  if(dims.chargingTime?.active)r.chargePerMinute=round(Number(dims.chargingTime.price||0)/60);
  if(dims.sessionTime?.active){r.afterMinutesRate=round(Number(dims.sessionTime.price||0)/60);r.afterMinutesThreshold=Math.max(0,Math.round(Number(dims.sessionTime.freeMins||0)));}
  if(dims.parkingTime?.active){r.postChargeRate=round(Number(dims.parkingTime.price||0)/60);r.postChargeGraceMinutes=Math.max(0,Math.round(Number(dims.parkingTime.freeMins||0)));}
  return{rule:r,connectorType:stat.connectorType||null,minPowerKw:stat.connectorPowerMinkW??null,maxPowerKw:stat.connectorPowerMaxkW??null};
}
function offer(id,provider,rules,extra={}){return{id,selectionId:id,provider,countries:['FR'],currency:'EUR',priority:130,pricing:{type:'rules',rules},source:'Load Motion matching-pricing-definitions/resolve',directOperatorOnly:true,verifiedScope:'exact_evse',defaultSelected:false,...extra};}
...
// MobiSDEC: tenant-wide power-band/time rules. parkingTime is post-charge occupation.
{
  ...
  for(const [id,d] of [...defs.entries()].sort((a,b)=>a[0].localeCompare(b[0]))){const x=ruleFromDefinition(d),kinds=x.connectorType==='T2'?['AC']:x.connectorType==='CCS'?['DC']:[];directOffers.push(offer(`loadmotion-mobisdec-${compactId(id).slice(-8).toLowerCase()}`,'MobiSDEC direct',[x.rule],{operatorAliases:aliases,networkAliases:aliases,connectorKinds:kinds,minPowerKw:x.minPowerKw,maxPowerKw:x.maxPowerKw,priority:128,directOperatorOnly:false,verifiedScope:'operator_or_network_power_time',metadata:{tenant:'mobisdec',definitionId:id,definitionName:d.name,parkingSemantics:'post-charge occupation; grace from parkingTime.freeMins'}}));}
}
