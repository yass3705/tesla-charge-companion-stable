...
def main():
 amap,_=aliases();done=treated(amap);counts=Counter()
 ...
 report={'totalCpoCount':total,'treatedCpoCount':len(treated_ops),'remainingCpoCount':total-len(treated_ops),'treatedShare':round(len(treated_ops)/total,6) if total else 0,'topUntreated':[{'operator':x,'siteRows':n} for x,n in untreated[:100]]}
