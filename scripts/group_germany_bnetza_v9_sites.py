...
def main():
 ...
 for key,items in groups.items():
  ...
  dates=[t(r[9]) for r in items if t(r[9])];updated=min(dates) if dates else ''
  outrows.append([sid,name,addr,lat,lon,op,cnt,access,cfg,updated,network])
  ...
  outcw.append({'canonicalId':f'DE:site:{sid}','siteId':sid,'bnetzaIds':facility_ids,'bnetzaEvseIds':sorted(set(all_evses or unique_evses)),'aliases':sorted(set(aliases+[f'bnetza:{z}' for z in facility_ids])),'operator':op,'rawOperators':sorted(rawops),'facilityCount':len(facility_ids)})
