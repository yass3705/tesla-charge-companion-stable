...
def aliases(p):
 ...
 return o
def dl(u):
 q=tempfile.NamedTemporaryFile(delete=False,suffix='.csv');q.close();req=urllib.request.Request(u,headers={'User-Agent':'TCC-V9-DE/1.1'})
...
def kind(c,p,typ):
 ...
 if any(x in s for x in ('typ_2','schuko','cee','normallade')):return 'AC'
...
def build(src,out,cross,alias_path,u):
 ...
 for r in reader(src):
  total+=1;lat=fnum(val(r,'Breitengrad'));lon=fnum(val(r,'Längengrad','Langengrad'))
