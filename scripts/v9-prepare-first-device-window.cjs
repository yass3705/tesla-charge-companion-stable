...
const now=new Date();const expires=new Date(now.getTime()+minutes*60000);const version=`device-test-${now.toISOString().replace(/[-:.TZ]/g,'').slice(0,14)}`;
...
const closeSelf={...clone(self),enabled:false,readinessApproved:false,tokenSha256:'',tokenVersion:version,expiresAt:null,maxGrantMinutes:Math.min(Number(self.maxGrantMinutes)||60,60),notes:'Closed after first-device test. No active token hash is retained.'};
