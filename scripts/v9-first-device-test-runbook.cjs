...
const closed =
  ...
  access.verdict==='BLOCKED' && access.ready===false && policy.enabled===false;
...
if(!closed && !controlledOpen){fail('repository is neither strict CLOSED nor valid CONTROLLED_OPEN state');return;}
...
console.log('Random canary remains 0% throughout. PASS, ROLLBACK or expiry all require CLOSE.');
