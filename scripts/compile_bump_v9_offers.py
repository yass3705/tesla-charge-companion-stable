...
def valid_hm(v):
    ...
    try:h=int(parts[0]);m=int(parts[1])
    except:return False
...
def normalize_time_bands(bands,value_key,extra_keys=()):
    if not isinstance(bands,list) or not bands:return None
    normalized=[];required={'start','end',value_key,*extra_keys}
    for band in bands:
        if not isinstance(band,dict) or set(band)!=required:return None
        start=band.get('start');end=band.get('end');value=norm_price(band.get(value_key))
        if not valid_hm(start) or not valid_hm(end) or value is None or value<0:return None
        row={'start':start,'end':end,value_key:value}
        for key in extra_keys:
            x=norm_price(band.get(key))
...
def build_exact_temporal_pricing(point):
    ...
    if not isinstance(rules,list) or not all(isinstance(r,dict) for r in rules):return None,None
    by_kind={r.get('kind'):r for r in rules};kinds=set(by_kind)
    if len(by_kind)!=len(rules):return None,None
    if kinds=={'minimum_total','energy','post_charge_occupancy'} and len(rules)==3:
        ...
        if any(v is None or v<0 for v in (minimum,energy,per_minute,grace)):return None,None
    ...
    if kinds=={'minimum_total','energy','flat_fee'} and len(rules)==3:
        ...
        allowed={'energy_above_kwh','session_duration_after_minutes'};condition_kinds=[c.get('kind') for c in conditions]
    ...
    if kinds=={'minimum_total','energy','session_duration_surcharge'} and len(rules)==3:
    ...
    if kinds=={'minimum_total','energy_time_bands'} and len(rules)==2:
        minimum_rule=by_kind['minimum_total'];bands_rule=by_kind['energy_time_bands']
        if set(minimum_rule)!={'kind','amountEur'} or set(bands_rule)!={'kind','bands'}:return None,None
        minimum=norm_price(minimum_rule.get('amountEur'));bands=normalize_time_bands(bands_rule.get('bands'),'eurPerKwh')
        if minimum is None or minimum<0 or not bands:return None,None
        pricing={'type':'rules','rules':[{'start':b['start'],'end':b['end'],'pricePerKwh':b['eurPerKwh']} for b in bands],'minimumTotalEur':minimum,'requiresSegmentationOnTariffBoundary':True}
        return pricing,'minimum_energy_time_bands'
    if kinds=={'minimum_total','energy','post_charge_occupancy_time_bands'} and len(rules)==3:
        minimum_rule=by_kind['minimum_total'];energy_rule=by_kind['energy'];bands_rule=by_kind['post_charge_occupancy_time_bands']
        if set(minimum_rule)!={'kind','amountEur'} or set(energy_rule)!={'kind','eurPerKwh'} or set(bands_rule)!={'kind','bands'}:return None,None
        minimum=norm_price(minimum_rule.get('amountEur'));energy=norm_price(energy_rule.get('eurPerKwh'));bands=normalize_time_bands(bands_rule.get('bands'),'eurPerMinute',('graceMinutes',))
        if any(v is None or v<0 for v in (minimum,energy)) or not bands:return None,None
        positive=[b for b in bands if b['eurPerMinute']>0];zero=[b for b in bands if b['eurPerMinute']==0]
        if len(positive)!=1 or not zero:return None,None
        charged=positive[0]
        if any(b['graceMinutes']!=0 for b in zero):return None,None
        pricing={'type':'rules','rules':[{'scope':'allDay','pricePerKwh':energy}],'postChargeFee':{'graceMinutes':charged['graceMinutes'],'eurPerMinute':charged['eurPerMinute'],'exemptLocalWindows':[{'start':b['start'],'end':b['end']} for b in zero]},'minimumTotalEur':minimum}
        if point.get('parkingText'):pricing['parkingText']=point.get('parkingText')
        return pricing,'minimum_energy_post_charge_occupancy_time_bands'
    return None,None
