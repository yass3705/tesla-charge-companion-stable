...
fast=providers('Fastned');ion=providers('IONITY');aral=providers('Aral pulse');enbw=providers('EnBW mobility+');swu=providers('SWU Energie');west=providers('Westfalen');eam=providers('EAM Natur Energie')
...
assert all(x.get('directOperatorOnly') is True for x in matches('SWU Energie'))
assert all(x.get('directOperatorOnly') is True for x in matches('Westfalen'))
assert all(x.get('directOperatorOnly') is True for x in matches('EAM Natur Energie'))
...
print(json.dumps({'Fastned':len(matches('Fastned')),'IONITY':len(matches('IONITY')),'Aral pulse':len(matches('Aral pulse')),'EnBW':len(matches('EnBW mobility+')),'SWU':len(matches('SWU Energie')),'Westfalen':len(matches('Westfalen')),'EAM':len(matches('EAM Natur Energie'))},indent=2))
