...
fast=providers('Fastned');ion=providers('IONITY');aral=providers('Aral pulse');enbw=providers('EnBW mobility+');swu=providers('SWU Energie');west=providers('Westfalen');eam=providers('EAM Natur Energie');bochum=providers('Stadtwerke Bochum');marke=providers('Mark-E')
...
for op in ('EnBW mobility+','SWU Energie','Westfalen','EAM Natur Energie','Stadtwerke Bochum','Mark-E'):
...
print(json.dumps({'Fastned':len(matches('Fastned')),'IONITY':len(matches('IONITY')),'Aral pulse':len(matches('Aral pulse')),'EnBW':len(matches('EnBW mobility+')),'SWU':len(matches('SWU Energie')),'Westfalen':len(matches('Westfalen')),'EAM':len(matches('EAM Natur Energie')),'Bochum':len(matches('Stadtwerke Bochum')),'Mark-E':len(matches('Mark-E'))},indent=2))
