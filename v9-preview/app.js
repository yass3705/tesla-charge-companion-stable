(async function(){
  ...
  $('load').addEventListener('click',async()=>{
    try{
      ...
      const session={energyKwh:Number($('energy').value||0),consumptionKwhPer100Km:Number($('consumption').value||15),batteryCapacityKwh:Number($('batteryCapacity').value||0)||null,startSoc:Number($('startSoc').value),targetSoc:Number($('targetSoc').value),vehicleMaxChargeKw:Number($('vehicleMaxKw').value||250),targetCurrency:'EUR',startAt:$('startAt').value||null,disconnectAt:$('disconnectAt').value||null,includeRouteEnergyInCharge:true};
