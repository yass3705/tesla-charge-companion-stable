(async function(){
  ...
  function priceLabel(o){const p=o.pricing||{},currency=o.currency||'EUR';if(p.pricePerKwh!=null)return `${Number(p.pricePerKwh).toFixed(3)} ${currency}/kWh`;const rs=p.rules||[];const kwh=[...new Set(rs.map(r=>r.pricePerKwh).filter(v=>v!=null))],minute=[...new Set(rs.map(r=>r.pricePerMinute).filter(v=>v!=null))];if(kwh.length)return `${kwh.map(v=>Number(v).toFixed(3)).join(' / ')} ${currency}/kWh`;if(minute.length)return `${minute.map(v=>Number(v).toFixed(3)).join(' / ')} ${currency}/min`;return p.type||'tarif complexe';}
  ...
  $('country').addEventListener('change',()=>{
    ...
    if(country==='NL'){$('lat').value='51.4416';$('lon').value='5.4697';$('radius').value='15';$('subCountries').value='NL';$('selectedSubscriptions').value='';}
    else if(country==='IT'){$('lat').value='41.9028';$('lon').value='12.4964';$('radius').value='15';$('subCountries').value='IT';$('selectedSubscriptions').value='atlante_go';}
    else if(country==='MA'){$('lat').value='33.5731';$('lon').value='-7.5898';$('radius').value='30';$('subCountries').value='MA';$('selectedSubscriptions').value='';}
    else{$('lat').value='48.798';$('lon').value='2.061';$('radius').value='15';$('subCountries').value='FR';$('selectedSubscriptions').value='';}
  });
  $('load').addEventListener('click',async()=>{
    try{
      ...
      const targetCurrency=countryCode==='MA'?'MAD':'EUR';
      const session={energyKwh:Number($('energy').value||0),startSoc:Number($('startSoc').value),targetSoc:Number($('targetSoc').value),targetCurrency,startAt:$('startAt').value||null,disconnectAt:$('disconnectAt').value||null,includeRouteEnergyInCharge:true};
