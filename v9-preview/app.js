(async function(){
  ...
  function render(area){
    const stations=area.rankedStations||area.stations||[];summary.textContent=`${(area.stations||[]).length} stations · ${area.operators?.length||0} opérateurs · ${area.subscriptions?.length||0} abonnements · ${area.diagnostics?.sessionComparableStationCount||0} tarifables · ${area.diagnostics?.fullyScoredStationCount||0} complètement scorées`;
  ...
  $('load').addEventListener('click',async()=>{
    try{
      setStatus('Chargement, fusion, simulation et scoring V9…');stationsEl.innerHTML='';
      ...
      const session={energyKwh:Number($('energy').value||0),consumptionKwhPer100Km:Number($('consumption').value||15),vehicleMaxChargeKw:Number($('vehicleMaxKw').value||250),targetCurrency:'EUR',startAt:$('startAt').value||null,disconnectAt:$('disconnectAt').value||null};
      ...
      render(area);setStatus(`Fusion, simulation et scoring terminés. ${area.diagnostics?.errors?.length||0} erreur(s) source.`,(area.diagnostics?.errors?.length||0)?'err':'ok');
