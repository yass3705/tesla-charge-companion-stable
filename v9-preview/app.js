(async function(){
  ...
  function render(area){
    const stations=area.rankedStations||area.stations||[];summary.textContent=`${(area.stations||[]).length} stations · ${area.operators?.length||0} opérateurs · ${area.subscriptions?.length||0} abonnements · ${area.diagnostics?.routedStationCount||0}/${area.diagnostics?.routingRequestedCount||0} routées · ${area.diagnostics?.fullyScoredStationCount||0} complètement scorées`;
  ...
  try{
    ...
    setStatus(`Moteur V9 prêt · ${Object.keys(loaders).length} sources · routage OSRM actif.`,'ok');
  ...
  $('load').addEventListener('click',async()=>{
    try{
      setStatus('Chargement, fusion, routage, simulation et scoring V9…');stationsEl.innerHTML='';
      ...
      const session={energyKwh:Number($('energy').value||0),consumptionKwhPer100Km:Number($('consumption').value||15),vehicleMaxChargeKw:Number($('vehicleMaxKw').value||250),targetCurrency:'EUR',startAt:$('startAt').value||null,disconnectAt:$('disconnectAt').value||null,includeRouteEnergyInCharge:true};
      const area=await engine.queryArea({countryCode,origin:{lat,lon},radiusKm,filters:{},subscriptionFilters:{minCountries:Number($('minCountries').value||1),countryCodes,coverageMode:'all'},selectedSubscriptions,session,sortBy:$('sortBy').value});
      render(area);const routeErrors=area.diagnostics?.routingErrorCount||0,sourceErrors=area.diagnostics?.errors?.length||0;setStatus(`Terminé · ${area.diagnostics?.routedStationCount||0} routes · ${sourceErrors} erreur(s) source · ${routeErrors} erreur(s) routage.`,sourceErrors||routeErrors?'err':'ok');
