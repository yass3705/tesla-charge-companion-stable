...
(function(){
  'use strict';
  const DATA_URL='data/freshmile_direct_tcc_v8.json.gz';
  const REVISION='rc48-freshmile-direct-20260825';
  ...
  function isFreshmileStation(st){
    return [st?.operator,st?._sourceOperator,st?.cpo,st?.network]
      .map(norm).filter(Boolean).some(v=>v==='freshmile'||v.startsWith('freshmile '));
  }
  ...
  function validateExact(exact){
    ...
    if(exact.free===true){
      if(exact.sessionFeeEur||energy||time)throw new Error('formule gratuite Freshmile incohérente');
      return true;
    }
    if(exact.sessionFeeEur!=null&&!(Number(exact.sessionFeeEur)>=0))throw new Error('frais de session Freshmile invalides');
    if(energy){
      if(!(Number(energy.amount)>=0)||!['started_kwh','linear_kwh'].includes(energy.billing))throw new Error('composante énergie Freshmile invalide');
    }
    if(time){
  ...
  function validateData(data){
    ...
    if(Number(counts.conflictingEvseExcluded)<1)throw new Error('garde-fou conflits Freshmile absent');
    return data;
  }
  ...
  async function loadData(){
    if(!dataPromise)dataPromise=fetch(`${DATA_URL}?v=20260825b`,{cache:'no-store'}).then(async r=>{
  ...
  function configKey(c){
    const exact=c?.pricing?.freshmileExact||null;
    return [norm(configProvider(c)),text(c?.kind).toUpperCase(),Number(c?.powerKw||0).toFixed(3),JSON.stringify(exact||c?.pricing||null)].join('|');
  }
  function mergeConfigurations(sources,direct=[]){
    const out=[],seen=new Set();
    for(const st of sources){
      for(const cfg of (Array.isArray(st?.chargingConfigurations)?st.chargingConfigurations:[])){
        const key=configKey(cfg);if(seen.has(key))continue;seen.add(key);out.push(clone(cfg));
      }
    }
  ...
  function locationAirKm(loc,origin){
    const values=[origin?.lat,origin?.lon,loc?.latitude,loc?.longitude].map(Number);
    return values.every(Number.isFinite)?haversineKm(...values):Infinity;
  }
  function stationAirKm(st,origin){
    const existing=Number(st?._airKm);if(Number.isFinite(existing))return existing;
    const values=[origin?.lat,origin?.lon,st?.latitude,st?.longitude].map(Number);
    return values.every(Number.isFinite)?haversineKm(...values):Infinity;
  }
  function inArea(data,prepared){
    const max=Math.max(0,Number(prepared?.maxDistanceKm||0)),origin=prepared?.origin||{};
    return (data?.stations||[]).map(st=>({...st,_airKm:locationAirKm(st,origin)}))
      .filter(st=>Number.isFinite(st._airKm)&&(max<=0||st._airKm<=max+1e-9));
  }
  function buildAssignments(base,official){
    ...
    for(const pair of pairs){
      if(assignedBase.has(pair.index)||assignedLocation.has(pair.locIndex))continue;
      assignedBase.add(pair.index);assignedLocation.add(pair.locIndex);byLocation.set(pair.locIndex,[pair]);
    }
  ...
  function canonicalFromMatches(loc,matches,base,origin){
    const primary=base[matches[0].index],sources=matches.map(x=>base[x.index]);
    const configurations=mergeConfigurations(sources,directConfigs(loc));
    const first=configurations.find(c=>c?.freshmileDirect)||configurations[0];
    return{
      ...primary,name:loc.name||primary.name,address:loc.address||primary.address,
      latitude:Number(loc.latitude),longitude:Number(loc.longitude),operator:'Freshmile',countryCode:'FR',
      kind:first?.kind||primary.kind,powerKw:Number(first?.powerKw||primary.powerKw||11),pricing:first?.pricing||primary.pricing,
      chargingConfigurations:configurations,stalls:Math.max(Number(primary.stalls||0),...(loc.configurations||[]).map(c=>Number(c.stalls||0))),
      _airKm:locationAirKm(loc,origin),freshmileStrictCpo:true,freshmileDirectStationId:loc.stationId,
      freshmileSourceCatalogStationIds:sources.map(x=>x.catalogStationId).filter(Boolean),freshmileStatusJoinedExternally:true,
      freshmileDirectConfigurationCount:(loc.configurations||[]).length,_freshmileOverlayRevision:REVISION
    };
  }
  function syntheticFromOfficial(loc,origin){
    const configs=directConfigs(loc),first=configs[0],id=`freshmile-direct:${loc.stationId}`;
    const stalls=new Set(configs.flatMap(c=>c.freshmileEvseIds||[])).size;
    return{
      id,catalogStationId:id,name:loc.name||'Station Freshmile',address:loc.address||'',operator:'Freshmile',
      latitude:Number(loc.latitude),longitude:Number(loc.longitude),countryCode:'FR',source:'freshmileDirectInventory',
      kind:first?.kind||'AC',powerKw:Number(first?.powerKw||11),stalls,pricing:first?.pricing||{type:'rules',rules:[]},chargingConfigurations:configs,
      access:{limited:false,unknown:true,days:{},afterCloseMode:'exit_allowed',afterCloseNote:'Horaires non fournis par la couche tarifaire Freshmile — accès à vérifier.'},
      temporarilyUnavailable:false,readOnlyCatalog:true,operationalStatus:'unknown',operationalStatusSource:'',
      _airKm:locationAirKm(loc,origin),freshmileStrictCpo:true,freshmileDirectStationId:loc.stationId,
      freshmileSourceCatalogStationIds:[],freshmileStatusJoinedExternally:false,freshmileDirectConfigurationCount:configs.length,_freshmileOverlayRevision:REVISION
    };
  }

  function mergePrepared(prepared,data){
    validateData(data);if(!prepared||!Array.isArray(prepared.stations))return prepared;
    if(prepared.freshmileDirectOverlayApplied&&prepared.freshmileDirectOverlayRevision===REVISION)return prepared;
    const base=prepared.stations.slice(),origin=prepared.origin||{},official=inArea(data,prepared);
    const {assignedBase,byLocation}=buildAssignments(base,official);
    const kept=base.filter((_,index)=>!assignedBase.has(index));
    const merged=[];let matched=0,added=0;
    official.forEach((loc,locIndex)=>{
      const matches=byLocation.get(locIndex)||[];
      if(matches.length){merged.push(canonicalFromMatches(loc,matches,base,origin));matched++;}
      else{merged.push(syntheticFromOfficial(loc,origin));added++;}
    });
  ...
  function installCandidateWrapper(){
    ...
    const wrapped=async function(filterMode='tesla',maxDistanceKm=0){
      ...
      if(filterMode!=='all'||!result||!Array.isArray(result.stations))return result;
      return applyToPrepared(result);
    };
    wrapped.__tccFreshmileDirectV1=true;wrapped.__tccOriginal=current;
    window.candidateStations=wrapped;try{candidateStations=wrapped}catch(e){}
    candidateInstalled=true;return true;
  }
  ...
  function occupiedMinutes(chargeMinutes,unplugTime,startTime){
    ...
    const a=toMin(startTime),b=toMin(unplugTime);if(!Number.isFinite(a)||!Number.isFinite(b))return charge;
    let delta=b-a;if(delta<0)delta+=1440;return Math.max(charge,delta);
  }
  ...
  function exactCost(pp,chargeMinutes,billedEnergy,unplugTime,startTime){
    ...
    const occupied=occupiedMinutes(chargeMinutes,unplugTime,startTime),energy=exact.energy||null,time=exact.time||null;
    const connection=Math.max(0,Number(exact.sessionFeeEur||0));
    ...
    if(time){const minutes=time.appliesTo==='occupied'?occupied:Math.max(0,Number(chargeMinutes||0));timeCost=startedUnits(minutes)*Number(time.amount||0);}
  ...
  function installPricing(){
    ...
    const wrapped=function(pp,startMinute,chargeMinutes,billedEnergy,unplugTime,startTime,powerSegments=[]){
      ...
      const base=current.apply(this,arguments);
      ...
      return{...base,total:exact.total,connection:exact.connection,chargeCost:exact.energyCost+(exact.timeAppliesTo==='charge'?exact.timeCost:0),
        connectedTimeCost:exact.timeAppliesTo==='occupied'?exact.timeCost:0,idleCost:0,durationSurcharge:0,occupiedMinutes:exact.occupiedMinutes,
        currencies:['EUR'],freshmileDirectPricing:true,freshmileExactPricing:true,freshmileEnergyCost:exact.energyCost,freshmileTimeCost:exact.timeCost};
    };
    wrapped.__tccFreshmileExactV1=true;wrapped.__tccOriginal=current;
    window.priceWithRules=wrapped;try{priceWithRules=wrapped}catch(e){}
    window.__TCC_FRESHMILE_PRICING_INSTALLED__=true;return true;
  }
