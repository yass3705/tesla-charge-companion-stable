...
(function(){
  ...
  function refreshTeslaIntro(){
    ...
    setTextIfChanged(title,'Base Tesla — données publiées automatiquement');
    setHtmlIfChanged(text,'Les Superchargeurs Tesla sont lus depuis <code>data/tesla_stations.json</code> et restent en lecture seule dans le Companion. Les corrections se font dans le Maintenance Center puis sont republiées dans la base.');
    const button=card.querySelector('button');
    if(button){
      setTextIfChanged(button,'Comment fonctionne la mise à jour');
      if(!button.dataset.augTeslaIntroBound){
        button.dataset.augTeslaIntroBound='1';
        button.onclick=()=>{
          const box=document.getElementById('teslaSyncInfo');
          setHtmlIfChanged(box,'Maintenance Center → validation des exports Tesla → publication de <code>tesla_stations.json</code> → lecture automatique par Tesla Charge Companion. Aucune modification locale d’une fiche Tesla n’est nécessaire.');
        };
      }
    }
  }
  ...
  function addLowSocWarnings(){
    ...
    document.querySelectorAll('.result-card .routeinfo').forEach(box=>{
      ...
      if(Number.isFinite(soc)&&soc<margin){
        ...
        warning.className='warn small aug-low-soc';
        warning.innerHTML=`<b>⚠ Arrivée sous la marge de sécurité réglée à ${margin.toFixed(0)} %.</b>`;
