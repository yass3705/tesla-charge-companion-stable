(function(root,factory){
  ...
  function normalizeConfig(raw={}){
    const stage=STAGES.includes(raw.stage)?raw.stage:'preview',canaryPercent=clamp(num(raw.canaryPercent)??0,0,100),productionPath=text(raw.productionPath)||'v9-app/';
    return{stage,canaryPercent,killSwitch:raw.killSwitch===true,requireReadiness:raw.requireReadiness!==false,salt:text(raw.salt)||'tcc-v9-rollout',previewPath:text(raw.previewPath)||'v9-preview/',canaryPath:text(raw.canaryPath)||productionPath,v8Path:text(raw.v8Path)||'./',productionPath};
  }
  ...
  function decide({config={},identity='',readiness=null,requestedStage=null}={}){
    ...
    if(requested==='canary'){
      if(b<c.canaryPercent)return{engine:'v9',stage:'canary',reason:'canary_bucket',bucket:b,readiness:ready,path:c.canaryPath};
