(function(root,factory){
  ...
  function createRegistryLoaders({registry,basePath='..',adapters={},fetchImpl}={}){
    ...
    for(const source of registry?.sources||[]){
      ...
      }else if(source.adapter==='france-emsp-compact-v1'&&adapters.franceEmspCompact?.createLoader&&source.root&&source.manifest){
        loaders[source.id]=adapters.franceEmspCompact.createLoader({base:join(basePath,source.root),manifestUrl:join(basePath,source.manifest),fetchImpl,source});
