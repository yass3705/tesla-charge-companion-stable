...
(function(){
  ...
  function pricingSig(p){
    if(!p)return'';
    const rules=(p.rules||[]).map(r=>({scope:r.scope||'',start:r.start||'',end:r.end||'',billing:r.billing||'',currency:(r.currency||'EUR').toUpperCase(),k:Number(r.pricePerKwh||0),m:Number(r.chargePerMinute||0),bc:Number(r.belibConnectedTimePerMinute||0),p:Number(r.parkingPerMinute||0),f:Number(r.connectionFee||0),i:Number(r.idlePerMinute||0),ar:Number(r.afterMinutesRate||0),at:Number(r.afterMinutesThreshold||0),pc:Number(r.postChargeRate||0),pg:Number(r.postChargeGraceMinutes||0)}));
  ...
  function sourceParts(pricing,time){
    ...
    if(Number(r.belibConnectedTimePerMinute||0)>0)parts.push(`branchement ${fmt(r.belibConnectedTimePerMinute)} ${c}/min`);
