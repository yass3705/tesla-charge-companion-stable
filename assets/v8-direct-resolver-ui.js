...
(function(){
  ...
  function powerState(){const s=readJson(POWER_KEY,{min:0,max:350})||{};let min=Math.max(0,Math.min(500,Number(s.min)||0)),max=Math.max(0,Math.min(500,Number(s.max)||350));if(min>max)[min,max]=[max,min];return{min,max}}
  ...
  function injectPowerWindow(){
    ...
    box.innerHTML=`<div class="v8-power-head"><div><b>Fenêtre de puissance</b><div class="small">Filtre supplémentaire au choix AC / DC</div></div><div id="v8PowerValue" class="v8-power-value"></div></div><div class="v8-power-track"><input id="v8PowerMin" type="range" min="0" max="350" step="1" value="${s.min}"><input id="v8PowerMax" type="range" min="0" max="350" step="1" value="${s.max}"></div><div class="v8-power-scale"><span>0 kW</span><span>350+ kW</span></div>`;
    if(kind)kind.parentElement?.insertAdjacentElement('afterend',box);else filters.prepend(box);
    const minEl=$('v8PowerMin'),maxEl=$('v8PowerMax'),val=$('v8PowerValue');const sync=changed=>{let min=Number(minEl.value),max=Number(maxEl.value);if(changed==='min'&&min>max){max=min;maxEl.value=String(max)}if(changed==='max'&&max<min){min=max;minEl.value=String(min)}val.textContent=`${min} — ${max>=350?'350+':max} kW`;savePowerState(min,max)};
    minEl.addEventListener('input',()=>sync('min'));maxEl.addEventListener('input',()=>sync('max'));sync();return true;
  }
  function installExpansionResolver(){
    ...
    const wrapped=function(baseStations){const source=Array.isArray(baseStations)?baseStations.map(addChargezyDirect):baseStations,expanded=current.call(this,source),p=powerState();return (expanded||[]).filter(st=>{const kw=Number(st?.powerKw||0);return Number.isFinite(kw)&&kw>=p.min-1e-9&&(p.max>=350||kw<=p.max+1e-9)})};
