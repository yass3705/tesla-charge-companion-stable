...
(function(){
  ...
  function writeSubscriptions(selected){localStorage.setItem(SUB_KEY,JSON.stringify({selected:[...selected],updatedAt:new Date().toISOString()}));if(window.TCCV8Subscriptions?.selectionChanged)window.TCCV8Subscriptions.selectionChanged();else window.TCCV8Subscriptions?.applyAll?.(true)}
