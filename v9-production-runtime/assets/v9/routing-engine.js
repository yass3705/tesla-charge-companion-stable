(function(root,factory){
  ...
  function withRequestTimeout(task,ms){let timer;const timeout=new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error('routing_request_timeout')),Math.max(1,ms));});return Promise.race([task,timeout]).finally(()=>clearTimeout(timer));}

  async function routeCandidates(stations,{origin,provider,session={},concurrency=6,requestTimeoutMs=6000,budgetMs=25000,signal}={}){
    ...
    const controller=typeof AbortController==='function'?new AbortController():null,started=Date.now();let cursor=0,timedOut=false,onAbort=null;
    if(controller&&signal?.aborted)controller.abort(signal.reason);
    else if(controller&&signal?.addEventListener){onAbort=()=>controller.abort(signal.reason);signal.addEventListener('abort',onAbort,{once:true});}
    ...
    async function worker(){
      while(cursor<rows.length){
        ...
        try{
          ...
          const raw=await withRequestTimeout(task,timeout);
          if(timedOut||controller?.signal?.aborted)break;
    ...
    try{await Promise.all(Array.from({length:Math.max(1,Math.min(Number(concurrency)||1,rows.length||1))},()=>worker()));}finally{clearTimeout(deadline);if(onAbort&&signal?.removeEventListener)signal.removeEventListener('abort',onAbort);}
