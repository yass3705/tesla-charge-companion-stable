(async function(){
  ...
  const status=$('status'),gatesEl=$('gates'),summaryEl=$('summary'),changesEl=$('changes'),matrixEl=$('matrix');let engine=null,v8=null;
  ...
  const parityOptions={compareOffers:false,costTolerance:.05,costRelativeTolerance:.01,socTolerancePoints:1,timeToleranceMinutes:2,criticalTimeToleranceMinutes:10,costPerKmTolerance:.001};
  ...
  function renderMatrix(matrix){
    ...
    matrixEl.querySelectorAll('.scenarioDetail').forEach(button=>button.addEventListener('click',()=>{
      const row=matrix.results[Number(button.dataset.index)];render(row.shadow);setStatus(`Scénario « ${row.label} » · ${row.classification}.`,row.classification==='regression'?'err':row.classification==='review'?'warn':'ok');
    }));
  }
  ...
  $('run').addEventListener('click',async()=>{
    try{
      ...
      const result=await window.TCCV9ParityEngine.shadowQuery({v8Query:q=>v8.query(q),v9Engine:engine,query:baseQuery(),options:parityOptions});
  ...
  $('runMatrix').addEventListener('click',async()=>{
    try{
      setStatus('Exécution séquentielle de la matrice V8↔V9…','warn');gatesEl.textContent='Matrice en cours…';summaryEl.textContent='';matrixEl.textContent='Calcul…';changesEl.textContent='Sélectionne un scénario après le calcul pour voir son détail.';
      const matrix=await window.TCCV9ParityScenarioEngine.runMatrix({baseQuery:baseQuery(),v8Query:q=>v8.query(q),v9Engine:engine,parityEngine:window.TCCV9ParityEngine,parityOptions});
      renderMatrix(matrix);setStatus(`Matrice terminée · ${matrix.summary.strictPassedCount}/${matrix.summary.strictScenarioCount} scénario(s) strict(s) passent · ${matrix.summary.matrixPass?'PASS':'FAIL'}.`,matrix.summary.matrixPass?'ok':'err');
    }catch(err){setStatus(`Matrice impossible : ${err.message}`,'err');gatesEl.textContent='Échec de la matrice.';matrixEl.textContent='';}
  });
})();
